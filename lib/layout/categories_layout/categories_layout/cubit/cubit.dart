...
import 'package:untitled1/model/payment/first_token.dart';
...
//=================updateQuantityOfInCartProduct=======================//
  ...
  Future GetFirstToken({String price}) {
    DioHelperPayment.postdata(
        url: 'auth/tokens',
        data: {'api_key': paymobApiKey}).then((value) {
      // var firstToken = FirstToken.fromJson(value.data);
      // paymobFirstToken=firstToken.token.toString();
      paymobFirstToken = value.data['token'];
      print('first token : ${paymobFirstToken}');
      getOrderId(price);
      emit(SuccessFirstPayment());
    })
        .catchError((error) {
      emit(ErrorFirstPayment(error.toString()));
    });
  }

  Future getOrderId(String price) {
    DioHelperPayment.postdata(
        url: 'ecommerce/orders',
        data: {'auth_token': paymobFirstToken,
      ...
      emit(SuccessOrderPayment());
    })
        .catchError((error) {
      emit(ErrorOrderPayment(error.toString()));
    });
  }
}
